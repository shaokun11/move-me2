...
export class Worker {
  ...
  async run({ perf }: { perf?: number } = {}) {
    ...
    const syncedVersion = await this.dataSource
    ...
    let startingVersion = BigInt(this.config.starting_version || 0n);
    if (syncedVersion) {
      const dbVersion = BigInt(syncedVersion.nextVersion);
      startingVersion =
        dbVersion > startingVersion ? dbVersion : startingVersion;
    }
    console.log(
