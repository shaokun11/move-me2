...
use crate::{
    ...
    common_transactions::{create_account_txn, peer_to_peer_evm_deposit_txn, peer_to_peer_evm_send_txn, peer_to_peer_txn},
...
pub static PEER_TO_PEER_EVM_SEND_TX: Lazy<u64> = Lazy::new(|| {
    // Compute gas used by running a placeholder transaction.
    let mut executor = FakeExecutor::from_head_genesis();
    let sender = AccountData::new(1_000_000, 10);
    let receiver = AccountData::new(1_000_000, 10);
