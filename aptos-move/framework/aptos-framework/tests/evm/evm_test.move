...
module aptos_framework::evm_test {
    ...
    public fun test_run() {
        ...
        let gas_limit = u256_to_data(0x017318);
        let gas_price = u256_to_data(0x0a);
        let value = u256_to_data(0x00);

        let storage_maps = simple_map::new<vector<u8>, simple_map::SimpleMap<vector<u8>, vector<u8>>>();
        let (storage_keys, storage_values) = (vector::empty<vector<vector<u8>>>(), vector::empty<vector<vector<u8>>>());
        simple_map::add(&mut storage_maps, x"b94f5374fce5edbc8e2a8697c15331677e6ebf0b", init_storage(vector[0x01,0x02], vector[0x01,0x01]
));
        ...
        let balance_table = vector[0xe8d4a51000, 0x00];
        let codes = vector[x"", x"6d6460016001556000526005601bf36000526000600e60126000f5503d6001556020600060003e60005160025500"];
