module aptos_framework::evm {
    ...
    struct TXHashEvent has drop, store {
        move_tx_hash: vector<u8>,
        evm_tx_hash: vector<u8>
    }

    struct CallEvent has drop, store {
        from: vector<u8>,
        to: vector<u8>,
        gas: u256,
        gas_used: u256,
    ...
    struct Global has key {
        execution_event: EventHandle<TXHashEvent>,
        trace: EventHandle<CallEvent>
    }
    ...
    native fun decode_raw_tx(
        raw_tx: vector<u8>
    ): (vector<u8>, u64, u64, vector<u8>, vector<u8>, u256, vector<u8>);
    ...
    public(friend) fun initialize(aptos_framework: &signer) {
        move_to(aptos_framework, Global {
            execution_event: new_event_handle<TXHashEvent>(aptos_framework),
            trace: new_event_handle<CallEvent>(aptos_framework)
        })
    }
    ...
    public entry fun send_tx(
        ...
        gas_bytes: vector<u8>,
        _tx_type: u64,
    ) acquires Account, ContractEvent, Global {
        let gas = to_u256(gas_bytes);
        let (evm_tx_hash, chain_id, nonce, evm_from, evm_to, value, data) = decode_raw_tx(tx);
    ...
    fun emit_trace(from: vector<u8>, to: vector<u8>, gas: u256, gas_used: u256, input: vector<u8>, value: u256, type: u8) acquires Global {
        emit_event(&mut borrow_global_mut<Global>(@aptos_framework).trace, CallEvent {
            from,
            to,
            gas,
            gas_used,
    ...
    fun execute(evm_from: vector<u8>, evm_to: vector<u8>, nonce: u64, data: vector<u8>, value: u256): vector<u8> acquires Account, ContractEvent, Global {
    ...
    fun run(sender: vector<u8>, origin: vector<u8>, evm_contract_address: vector<u8>, code: vector<u8>, data: vector<u8>, readOnly: bool, value: u256, transient: &mut SimpleMap<u256, u256>): (bool, vector<u8>) acquires Account, ContractEvent, Global {
        ...
        while (i < len) {
            ...
            if(opcode == 0x00) {
                ...
                let(neg_a, num_a) = to_int256(vector::pop_back(stack));
                let(neg_b, num_b) = to_int256(vector::pop_back(stack));
                ...
                *vector::borrow_mut(memory, (pos as u64)) = ((value & 0xff) as u8);
                ...
                let call_type = if (opcode == 0xf1) 0 else if(opcode == 0xfa) 1 else 2;
                ...
                if (is_precompile_address(evm_dest_addr) ||exist_contract(move_dest_addr)) {
                    ...
                    emit_trace(from, target, m_pos, m_len, params, msg_value, call_type);
    ...
    fun test_pancake_v3(aptos_framework: signer) acquires Account, ContractEvent, Global {
        // debug::print(&exp(2, 5));
        ...
        debug::print(&utf8(b"get pool"));
        let get_pool_data = &mut x"1698ee82";
        ...
        // estimate_tx_gas(sender, manager, *add_liquidity_data, u256_to_data(amount_to_mint), 1);
