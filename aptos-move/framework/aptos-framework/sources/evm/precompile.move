module aptos_framework::precompile {
    use std::vector;
    use aptos_framework::evm_util::{slice, to_u256, to_32bit_leading_zero};
    ...
    public fun run_precompile(addr: vector<u8>, calldata: vector<u8>, chain_id: u64): vector<u8> {
        if(addr == RCRECOVER) {
            ...
            to_32bit_leading_zero(slice(pk, 12, 20))
        } else if(addr == SHA256) {
            sha2_256(calldata)
        } else if(addr == RIPEMD) {
            debug::print(&to_32bit_leading_zero(ripemd160(calldata)));
            to_32bit_leading_zero(ripemd160(calldata))
    ...
    // use aptos_std::ristretto255::{scalar_add, new_scalar_from_bytes};
    // use aptos_std::secp256k1;
    // use aptos_std::crypto_algebra;

    // fun read_point(x: vector<u8>, y:vector<u8>) {
    //
    // }



    // fun ec_add(ax: vector<u8>, ay:vector<u8>, bx:vector<u8>, by:vector<u8>): vector<u8> {
    //     let pa = new_scalar_from_bytes(ax);
    //     crypto_algebra
    //     scalar_add(ax, ay, bx, by);
    //     secp256k1
    // }
