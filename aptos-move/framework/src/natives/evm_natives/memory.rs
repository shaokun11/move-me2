...
impl Memory {
	...
	/// does nothing.
	pub fn resize_offset(&mut self, offset: U256, len: U256) -> Result<(), ExecutionError> {
		if len == U256::zero() {
			return Ok(());
		}
		...
		offset
			.checked_add(len)
			.map_or(Err(ExecutionError::InvalidRange), |end| self.resize_end(end))
	}

	/// Resize the memory, making it cover to `end`, with 32 bytes as the step.
	pub fn resize_end(&mut self, end: U256) -> Result<(), ExecutionError> {
		if end > self.effective_len {
			let new_end = next_multiple_of_32(end).ok_or(ExecutionError::InvalidRange)?;
